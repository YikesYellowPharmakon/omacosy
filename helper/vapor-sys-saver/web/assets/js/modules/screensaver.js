...
const WTE_WASM_URL = new URL('../wte/ttfx.wasm', import.meta.url);
...
const EXCLUDE_EFFECTS = new Set();

function logSaver(...args) {
  const line = args.map((item) => (item instanceof Error ? item.stack || item.message : String(item))).join(' ');
  console.log('[vapor-sys]', line);
  try {
    window.webkit?.messageHandlers?.log?.postMessage(line);
  } catch {
  }
}
...
function markStatic() {
  document.documentElement.classList.add('screensaver--static');
}

function ready() {
  const canvas = document.getElementById('screensaver');
  const pre = document.getElementById('screensaver-art');
  ...
  afterFonts()
    .then(loadWasm)
    .then((bytes) => {
      logSaver('wasm bytes', String(bytes.byteLength));
      return init({ module_or_path: bytes });
    })
    .then(() => {
      const names = catalogNames(effect_catalog()).filter(
        (name) => !EXCLUDE_EFFECTS.has(name.toLowerCase()),
      );
      logSaver('effects', names.join(','));
      if (names.length === 0) {
      ...
      layout();
      startEffect(picker.pick());
      logSaver('start', currentEffect, metrics.columns, 'x', metrics.rows);
      paint(performance.now());
      play();
